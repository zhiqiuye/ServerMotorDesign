...
{
	float	theta_rad;
	theta_rad							=	m_motor_rt_para.m_park.f_theta_rad;					//转子电角度转化为弧度，用于计算正余弦
...
void	RotorDetect(void)
{
	...
	m_motor_rt_para.m_park.f_theta_rad			+=	(temp_delta * m_motor_attribute_para.m_motor_ind_att.f_rad_per_cnts);
}
	...
	/*---------------------------------------------------------------------------
	函数名称			：	RotorCorrection(uint8_t hall_state)
	参数含义			：	null
	函数功能			：	更新转子位置信息，theta值
						在Hall捕获中断中进行矫正处理
	----------------------------------------------------------------------------*/
...
void	CheckSector(void)
{
	...
	if(temp_u1 > 0.00000001f)	m_motor_rt_para.m_revpark.u8_vref_sector |= 0x01;
	else	m_motor_rt_para.m_revpark.u8_vref_sector &= 0xFE;
	if(temp_u2 > 0.00000001f)	m_motor_rt_para.m_revpark.u8_vref_sector |= 0x02;
	else	m_motor_rt_para.m_revpark.u8_vref_sector &= 0xFD;
	if(temp_u3 > 0.00000001f)	m_motor_rt_para.m_revpark.u8_vref_sector |= 0x04;
	else	m_motor_rt_para.m_revpark.u8_vref_sector &= 0xFB;
	...
	switch (m_motor_rt_para.m_revpark.u8_vref_sector)
	{
		case 1:				//扇区号2
			...
			Tbon=	Taon + t2 * 0.5f;
			Tcon=	Tbon + t6 * 0.5f;
			m_motor_rt_para.m_reverse.i16_T_1	=	(uint16_t)Taon;
			m_motor_rt_para.m_reverse.i16_T_2	=	(uint16_t)Tcon;
			m_motor_rt_para.m_reverse.i16_T_3	=	(uint16_t)Tbon;
			break;
...
void	FOC_Cal(void)
{
	...
	RotorDetect();
	/*电流环PI计算*/
	m_motor_rt_para.m_revpark.f_v_q		=	Increment_PI_Cal(&(m_pid.iq),m_motor_ctrl.m_motion_ctrl.f_set_iq);
	m_motor_rt_para.m_revpark.f_v_d		=	Increment_PI_Cal(&(m_pid.id),m_motor_ctrl.m_motion_ctrl.f_set_id);
	/*park逆变换*/
	ReverseParkConvert();
	...
	/*更改PWM占空比*/
	TIM1->CCR1				=	(uint16_t)m_motor_rt_para.m_reverse.i16_T_1;
	TIM1->CCR2				=	(uint16_t)m_motor_rt_para.m_reverse.i16_T_2;
	TIM1->CCR3				=	(uint16_t)m_motor_rt_para.m_reverse.i16_T_3;
}
